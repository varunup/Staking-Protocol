...
contract StakingProtocol is IStakingProtocol, Ownable {
    ...
    uint256 public cliff; //Change variable name to more approprite.
    ...
    function changeStakingStatus(bool stakingStatus_) public virtual override onlyOwner returns (bool) {
        paused = stakingStatus_;
        return true;
    }

    function changePoolStatus(uint256 poolId, bool poolStatus_) public virtual override onlyOwner returns (bool) {
    ...
    function createPool( address stakingToken, address rewardToken, uint256 yieldPerSecond ) public virtual override onlyOwner returns (bool) {
        require(stakingToken != address(0) && rewardToken != address(0), "Invalid Stake Or Reward Token Address");
        poolCount += 1;
        pool[poolCount] = PoolInfo(rewardToken, stakingToken, yieldPerSecond, 0);
    ...
    function stakeToken(uint256 poolId, uint256 amount) external virtual override returns (bool) {
        ...
        require(IERC20(tokenAddress).allowance(_msgSender(), address(this)) >= amount, "Not Have Enought Allowance");
    ...
    function claimToken(uint256 poolId, bool unStaking) public virtual override returns (bool) {
        require(!paused, "Staking Is Paused");
        uint256 unclaimedReward = fetchUnclaimedReward(poolId);
